...
#define INTERVAL        100                    // Publish Interval[Sec]
#define MQTT_TOPIC      "/XBEE-MQTT/"          // Publish Topic
...
void setup() {
  ...
  Serial.println("Start MQTT Publish [" + String(_MODEL_) + "]");
...
void loop() {
  ...
  if ( (now - lastSendPacketTime) > 1000) { // 1 second elapsed
    lastSendPacketTime = now;
    timer++;
    ...
    if (timer >= INTERVAL) { // Publish
      ...
      packet_size = buildPublish(packet, MQTT_TOPIC, msg);
      if (_DEBUG_) hexDump(packet, packet_size);
      _SERIAL_.write(packet, packet_size);
      timer = 0;
    }
